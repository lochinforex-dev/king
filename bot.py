...
# --- 2. BARCHA VILOYAT VA TUMANLAR (HECH QAYSI QOLIB KETMADI) ---
REGIONS_DATA = {
    "Toshkent": {"Toshkent sh.": 0, "Angren": -3, "Bekobod": 2, "Chirchiq": -2, "G'azalkent": -3, "Piskent": 2, "Yangiyo'l": 2, "Olmaliq": 1, "Buka": 4, "Chinoz": 3, "Oqqo'rg'on": 2, "Parkent": -1, "Zangiota": 0, "Qibray": -1, "Yuqorichirchiq": -2, "O'rtachirchiq": 1, "Quyichirchiq": 3},
    ...
    "Namangan": {"Namangan sh.": -10, "Chust": -8, "Pop": -6, "Uychi": -11, "Uchqo'rg'on": -12, "Kosonsoy": -9, "Mingbuloq": -8, "To'raqo'rg'on": -9, "Chartaq": -11, "Norin": -11, "Yangiqo'rg'on": -10},
    "Sirdaryo": {"Guliston": 4, "Yangiyer": 5, "Sirdaryo": 3, "Boyovut": 4, "Sardoba": 6, "Oqoltin": 6, "Xovos": 5, "Sayhunobod": 3, "Mirzaobod": 5},
    "Jizzax": {"Jizzax sh.": 6, "Zomin": 4, "Do'stlik": 8, "G'allaorol": 7, "Paxtakor": 7, "Mirzachul": 9, "Arnasoy": 9, "Baxmal": 8, "Forish": 10, "Zarbdor": 7, "Yangiobod": 5},
    "Samarqand": {"Samarqand sh.": 9, "Kattaqo'rg'on": 12, "Urgut": 8, "Ishtixon": 10, "Bulung'ur": 8, "Narpay": 12, "Payariq": 10, "Jomboy": 9, "Nurobod": 11, "Oqdaryo": 10, "Pastdarg'om": 10, "Toyloq": 9, "Qo'shrabot": 11, "Paxtachi": 13},
    "Qashqadaryo": {"Qarshi": 18, "Shahrisabz": 15, "Muborak": 23, "Kasbi (Mug'lon)": 20, "Kitob": 14, "G'uzor": 19, "Chiroqchi": 16, "Dehqonobod": 18, "Koson": 20, "Nishon": 21, "Mirishkor": 24, "Ko'kdala": 17, "Yakkabog'": 15, "Qamashi": 17},
    "Surxondaryo": {"Termiz": 16, "Denov": 11, "Sherobod": 18, "Sho'rchi": 12, "Boysun": 14, "Sariosiyo": 11, "Jarqo'rg'on": 15, "Qumqo'rg'on": 13, "Muzrobot": 19, "Oltinsoy": 12, "Uzun": 10, "Qiziriq": 15, "Bandixon": 14, "Angor": 17},
    "Buxoro": {"Buxoro sh.": 24, "Gijduvon": 23, "Olot": 28, "Qorako'l": 27, "Vobkent": 23, "Gazli": 27, "Qorovulbozor": 25, "Shofirkon": 24, "Kogon": 24, "Jondor": 25, "Peshku": 24, "Romitan": 25},
    "Navoiy": {"Navoiy sh.": 16, "Zarafshon": 22, "Uchquduq": 24, "Nurota": 18, "Xatirchi": 14, "Qiziltepa": 17, "Tomdi": 22, "Karmana": 16, "Konimex": 18, "Navbahor": 15},
    "Xorazm": {"Urganch": 40, "Xiva": 41, "Hazorasp": 38, "Gurlan": 41, "Shovot": 42, "Xonqa": 40, "Bog'ot": 39, "Yangiariq": 41, "Yangibozor": 42, "Qo'shko'pir": 43, "Tuproqqal'a": 36},
...
# --- 3. TO'LIQ VAQT JADVALI (20-MART HAM BOR) ---
RAMADAN_CALENDAR = {
...
async def start_handler(message: types.Message):
    ...
    builder.adjust(1)
    await message.answer("🌙 **Ramazon 2026**\nO'zbekistonning barcha tumanlari kiritilgan eng to'liq bot.", reply_markup=builder.as_markup())
...
async def region_handler(call: types.CallbackQuery):
    ...
    builder.adjust(2) # Tumanlar ko'p bo'lsa ham, Telegram buni chiroyli tartiblaydi
    await call.message.edit_text(f"📍 **{region}**. Tumanlar ro'yxati:", reply_markup=builder.as_markup())
...
async def full_calendar_handler(call: types.CallbackQuery):
    ...
    # Birinchi 15 kun
    res1 = f"📅 **{district} (1-qism)**\nK | Sana | Sahar | Iftor\n"
    for i, date_str in enumerate(sorted(RAMADAN_CALENDAR.keys())[:15], 1):
        t = get_times(date_str, offset)
        res1 += f"{i} | {date_str[8:]}-{date_str[5:7]} | **{t['sahar']}** | **{t['iftor']}**\n"
    
    # Ikkinchi 15-16 kun
    res2 = f"📅 **{district} (2-qism)**\nK | Sana | Sahar | Iftor\n"
    for i, date_str in enumerate(sorted(RAMADAN_CALENDAR.keys())[15:], 16):
        t = get_times(date_str, offset)
        res2 += f"{i} | {date_str[8:]}-{date_str[5:7]} | **{t['sahar']}** | **{t['iftor']}**\n"

    await call.message.answer(res1, parse_mode="Markdown")
    builder = InlineKeyboardBuilder()
    builder.button(text="🏠 Menu", callback_data="to_start")
    await call.message.answer(res2, parse_mode="Markdown", reply_markup=builder.as_markup())
